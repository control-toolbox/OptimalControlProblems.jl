...
"""
function OptimalControlProblems.electric_vehicle(::JuMPBackend; nh::Int=500)
    ...
    @constraints(
        model,
        begin
            ∂x[t=1:nh], x[t] == x[t - 1] + 0.5 * step * (dx[t - 1] + dx[t])
            ∂v[t=1:nh], v[t] == v[t - 1] + 0.5 * step * (dv[t - 1] + dv[t])
        end
    )
